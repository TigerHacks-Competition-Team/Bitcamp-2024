...
{#await getData() then juser}
            ...
            <Command.List>
                ...
                <Command.Group>
                    {#each juser.friends as jfriend}
                        <Command.Item onSelect={() => currentFriends[jfriend.id] = jfriend}>
    ...
    {#each Object.values(currentFriends) as friend}
        <Card class="p-1 m-1">
            ...
            <Input on:input={e => currentFriends[friend.id].due = parseInt(e?.target?.value)} placeholder="Amount Due..." />
        </Card>
    {/each}

    <Input bind:value={poolName} />
